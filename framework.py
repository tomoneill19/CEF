...
def removeFromFile(match, fname):
    f = open(fname, "r")
    data = f.readlines()
...
def initIntel():
    ...
    try:
        ...
        lists = listfile.readlines()
        for line in lists:
            data = line.replace("\n","").split(" ")
            listName = data[0]
            ipNames.append(listName)
            customLists.append([])
            for x in range(1, len(data)):
                customLists[-1].append(int(data[x]))
        listfile.close()
    except:
        listfile = open("hosts.txt", "w")
        listfile.write("")
        listfile.close()

    try:
        intelfile = open("intel.txt", "r")
        intelines = intelfile.readlines()
        for line in intelines:
            data = line.replace("\n", "").split("|")
            subject = data[0]
            intelList = []
            for x in range(1, len(data)):
                if len(data[x] > 1):
                    intelList.append(data[x])
            intel[subject] = intelList
        intelfile.close()
    except:
        intelfile = open("intel.txt", "w")
        intelfile.write("")
        intelfile.close()

def writeIntel(subject, info):
    intelfile = open("intel.txt", "r")
    data = intelfile.readlines()
    found = False
    for x in range(0, len(data)):
        split = data[x].split("|")
        if split[0] == subject:
    ...
    intelfile.close()
    os.remove("intel.txt")
    intelfile = open("intel.txt", "a")
    for line in data:
        intelfile.write(line)
    if not found:
        intelfile.write("%s|%s\n" % (subject, info))
    intelfile.close()

def removeIntel(subject, info):
    intelfile = open("intel.txt", "r")
    lines = intelfile.readlines()
    intelfile.close()
    newline = ""
    for x in range(0, len(lines)):
        split = lines[x].split("|")
        if split[0] == subject and (info in split or (info + '\n') in split):
            try:
                split.remove(info)
            except:
                split.remove(info + '\n')
            newline = "|".join(split)
            print(newline)
            removeFromFile(lines[x], "intel.txt")
            writeToFile(newline, "intel.txt")

def writeList(listname, target):
    listfile = open("lists.txt", "r")
    ...
    if split != []:
        if target not in split:
            split.append(target)
            removeFromFile(old, "lists.txt")
            writeToFile(newline, "lists.txt")
